...
const atStars = () => {
  ...
  for (let i = 0; i < positions.length; i += 3) {
    ...
    /*-gap < x < gap */
    let x = Math.random() * gap * (Math.random() < 0.5 ? -1 : 1)
    let y = Math.random() * gap * (Math.random() < 0.5 ? -1 : 1)
    let z = Math.random() * gap * (Math.random() < 0.5 ? -1 : 1)
    ...
    positions[i] =
      gap * Math.sin(Math.acos(Math.abs(z) / gap)) * Math.cos(Math.atan(Math.abs(y) / Math.abs(x))) * (x / Math.abs(x))
    positions[i + 1] =
      gap * Math.sin(Math.acos(Math.abs(z) / gap)) * Math.sin(Math.atan(Math.abs(y) / Math.abs(x))) * (y / Math.abs(y))
...
const backStars = () => {
  ...
  for (let i = 0; i < positions.length; i += 3) {
    ...
    /*-gap < x < gap */
    let x = Math.random() * 6 * gap * (Math.random() < 0.5 ? -1 : 1)
    let y = Math.random() * 6 * gap * (Math.random() < 0.5 ? -1 : 1)
    let z = Math.random() * 6 * gap * (Math.random() < 0.5 ? -1 : 1)
    ...
    // positions[ i + 1 ] = y;
    // positions[ i + 2 ] = z;
    const tempGap = Math.sqrt(x * x + y * y + z * z) > 6 * gap ? 6 * gap : Math.sqrt(x * x + y * y + z * z)

    positions[i] =
      tempGap *
      Math.sin(Math.acos(Math.abs(z) / tempGap)) *
      Math.cos(Math.atan(Math.abs(y) / Math.abs(x))) *
      (x / Math.abs(x))
    positions[i + 1] =
      tempGap *
      Math.sin(Math.acos(Math.abs(z) / tempGap)) *
      Math.sin(Math.atan(Math.abs(y) / Math.abs(x))) *
      (y / Math.abs(y))
